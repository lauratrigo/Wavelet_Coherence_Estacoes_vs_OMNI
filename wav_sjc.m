...
load('mediasionosfericasSJC.mat');  % contém foF2, hF, hmF2
...
outputFolder = 'figuras_wcoh_sjc';
if ~exist(outputFolder, 'dir')
    mkdir(outputFolder);
end

% --- Definir nomes corretamente
nome_iono = iono_names{i};
nome_vd   = omni_names{j};


% --- Remover caracteres inválidos para nomes de arquivos
nome_iono_safe = regexprep(nome_iono, '[^\w]', '_');
nome_vd_safe   = regexprep(nome_vd,   '[^\w]', '_');

% --- Criar nome do arquivo (sem espaços nem caracteres especiais)
filename = sprintf('WCOH_SJC_%s_%s', nome_iono_safe, nome_vd_safe);
filename = fullfile(outputFolder, filename);

% --- Salvar em PNG (300 dpi)
print(gcf, [filename '.png'], '-dpng', '-r300');

% --- Salvar em EPS (vetorial)
print(gcf, [filename '.eps'], '-depsc');
